...
class EmailService {
    constructor() {
        ...
        this.apiKey = index_1.config.brevo.apiKey;
        this.sender = {
            email: index_1.config.brevo.senderEmail,
            name: index_1.config.brevo.senderName,
        };
    ...
    async sendVerificationEmail(email, name, verificationToken) {
        const verificationLink = `${index_1.config.frontendUrl}/verify-email?token=${verificationToken}`;
        const emailData = {
            sender: this.sender,
            to: [{ email, name }],
    ...
    async sendPasswordResetEmail(email, name, resetToken) {
        const resetLink = `${index_1.config.frontendUrl}/reset-password?token=${resetToken}`;
        const emailData = {
            sender: this.sender,
            to: [{ email, name }],
    ...
    async sendWelcomeEmail(email, name) {
        const loginLink = `${index_1.config.frontendUrl}/login`;
        const emailData = {
            sender: this.sender,
            to: [{ email, name }],
    ...
    async sendTaskAssignmentEmail(email, name, taskTitle, taskDescription, assignedByName, dueDate, workspaceName) {
        const dashboardLink = `${index_1.config.frontendUrl}/dashboard/tasks`;
        const emailData = {
            sender: this.sender,
            to: [{ email, name }],
