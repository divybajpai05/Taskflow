...
class WorkspaceService {
    ...
    async deleteWorkspace(workspaceId, userId) {
        ...
        await drizzle_1.db.delete(schema_1.leaves).where((0, drizzle_orm_1.eq)(schema_1.leaves.workspaceId, workspaceId));
        ...
        await drizzle_1.db.delete(schema_1.tasks).where((0, drizzle_orm_1.eq)(schema_1.tasks.workspaceId, workspaceId));
        ...
        const membersToDelete = await drizzle_1.db
        ...
        if (userIds.length > 0) {
            ...
            for (const uid of userIds) {
                const [otherMembership] = await drizzle_1.db
                    .select()
                    .from(schema_1.workspaceMembers)
                    .where((0, drizzle_orm_1.eq)(schema_1.workspaceMembers.userId, uid))
                    .limit(1);
                if (!otherMembership) {
                    await drizzle_1.db
                        .delete(schema_1.userPermissions)
                        .where((0, drizzle_orm_1.eq)(schema_1.userPermissions.userId, uid));
                    // Delete user if no other workspace memberships
                    await drizzle_1.db.delete(schema_1.users).where((0, drizzle_orm_1.eq)(schema_1.users.id, uid));
                }
            }
        }
