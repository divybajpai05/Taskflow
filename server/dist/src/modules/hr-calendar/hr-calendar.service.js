...
class HRCalendarService {
    ...
    async getHREvents(workspaceId, userId, userPermissions, userTeamId, month, year) {
        ...
        const attendanceEvents = await this.getDailyAttendanceSummary(workspaceId, userId, startDate, endDate, true, // canManageAll = true (show all)
        []);
        const leaveEvents = await this.getLeaveEvents(workspaceId, userId, startDate, endDate, true, // canManageAll = true (show all)
        []);
    ...
    async getLeaveEvents(workspaceId, userId, startDate, endDate, canManageAll, teamMemberIds) {
        ...
        const approvedLeaves = await drizzle_1.db
            ...
            .leftJoin(schema_1.teams, (0, drizzle_orm_1.eq)(schema_1.users.teamId, schema_1.teams.id))
